...
**Generic ACP** (`acp_runtime.py`) speaks newline-delimited JSON-RPC (single messages or batches) to a root-owned launcher declared by a bounded, root-owned manifest in the derived image. It implements the ACP v1, environment-authenticated baseline: the initialize response must select v1, auth methods require `id` + `name`, terminal auth is rejected because Trinity advertises `auth.terminal=false`, and a session that actually requires interactive protocol login fails explicitly. Trinity advertises no reverse filesystem or terminal methods, because harnesses use their own tools against the mounted workspace. It retains one process for Chat continuity and isolates every headless task. Progressive tool updates remain open until a terminal status and are fail-closed during prompt cleanup; only `end_turn` succeeds. `usage_update` provides authoritative context telemetry and optional per-turn USD cost deltas when emitted. Operator termination sends `session/cancel` and waits for `cancelled` before process-group fallback. Malformed or over-limit streams poison and discard the retained connection. The provider model is propagated as `ACP_MODEL`; common wall-clock guardrails, pre-publication sanitization, and error→HTTP mapping are enforced by Trinity. MCP, persisted Session-tab resume, image input, portable per-tool restrictions, and guaranteed cost reporting are not advertised. Hermes/Gemini and DeepSeek Harness are version-pinned acceptance harnesses built on the current Trinity base image, not runtime branches or fully hermetic artifacts. See [acp-runtime.md](feature-flows/acp-runtime.md).
