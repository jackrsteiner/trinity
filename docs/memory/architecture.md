...
A Trinity **harness IS an `AgentRuntime`** — the pluggable execution engine inside the agent container. Four ship today: **Claude Code** (default), **Gemini CLI**, **OpenAI Codex** (#1187), and the **generic ACP adapter** (ACP-001 — one protocol runtime for any conforming Agent Client Protocol agent, not a per-harness adapter). `AGENT_RUNTIME` (container env, set from `template.yaml runtime:` via `crud.py`; also a `trinity.agent-runtime` label) selects one; `runtime_adapter.get_runtime()` is the factory — it **validates** the value against `KNOWN_RUNTIMES` and raises on an unknown one rather than silently defaulting to Claude. That fail-loud contract is for **execution** paths only: read surfaces (`/health`, `/api/chat/session`, `GET /api/model`) go through `runtime_adapter.get_capabilities_snapshot()`, which degrades to permissive legacy defaults on a construction failure — `/health` is unauthenticated and load-bearing (monitoring, dispatch-breaker `consecutive_failures`, `clone_status`, readiness gates), so a runtime config typo must never turn it into a 500.

**ABC** (`agent_server/services/runtime_adapter.py`): `execute` (chat), `execute_headless` (stateless task), `configure_mcp`, `is_available`, `get_default_model`, `get_context_window`, plus a non-abstract `capabilities()` returning a `RuntimeCapabilities` dataclass (`chat_continuity`, `session_tab_resume`, `session_load`, `mcp_support`, `model_selection`, `system_prompt`, `tool_restrictions`, `prompt_images`/`prompt_audio`, `cost_reporting: "native"|"estimated"|"unavailable"`, `negotiated`) — conservative by default (an un-overridden runtime is least-capable). Non-abstract lifecycle hooks with no-op defaults: `get_capabilities()` (instance snapshot — a protocol runtime overrides it post-negotiation), `cancel_execution()` (protocol-native cancel; `False` keeps the process-registry signal path), `reset_chat()` (chat reset must reach a runtime holding a live child process), `close()` (server shutdown). Each runtime is a singleton (`get_<name>_runtime()`).
...
**Generic ACP** (`acp_runtime.py` + `acp_launch.py`, ACP-001; ADR [0002](../adr/0002-generic-acp-runtime.md), flow [acp-runtime.md](feature-flows/acp-runtime.md)): a harness-neutral client over the official `agent-client-protocol` SDK (exact-pinned 0.12.1, base image + `tests/requirements-test.txt`). Launch envelope is generic data — `AGENT_RUNTIME_COMMAND` + JSON `AGENT_RUNTIME_ARGS` (shell-free, NUL/type/count-vetted at BOTH ends: backend `helpers.validate_acp_launch` at create → named 400s incl. `runtime: acp` without a command and the unsupported `model:` override; agent-side `ACPLaunchConfig` re-validates). Lifecycle: spawn → `initialize`/version check → capability snapshot (`negotiated=True`; `session/load` only when advertised) → `session/new|load` → `session/prompt` with typed streamed updates → protocol `session/cancel` (bounded; undeliverable → `False` → registry signal fallback) → bounded shutdown. **No emulation**: model selection / system prompts / tool restrictions / max-turns are refused (`ACPFeatureUnavailable`, an HTTPException **409**), cost is `"unavailable"` (never estimated), and the backend gates the platform prompt off for ACP at both dispatch layers (`gate_system_prompt` in `task_execution_service` AND `chat_execution_service` — sync `/chat` is what MCP `chat_with_agent` uses). **Read-only mode fails CLOSED** — ACP has no portable enforcement channel, so an enabled `~/.trinity/read-only-config.json` refuses the turn (409) rather than running unenforced; guardrails are likewise not wired and turns are refused-or-honest, never silently unprotected (corrupt/unreadable config stays fail-open + WARN, Codex loader parity). Error→HTTP: capability refusals→**409**, protocol/agent/process failures→**502** (AGENT_ERROR — deliberately never 503/429, which the backend reads as AUTH/rate signals the adapter cannot portably prove), timeout→504. Chat continuity holds ONE live child process (`/api/chat`'s execution lock serializes it); per-turn collector reset keeps `cost_usd` from re-accumulating; `DELETE /api/chat/history` → `reset_chat()` closes it. `"acp"` is in `RUNTIMES_WITHOUT_SESSION_TAB_RESUME`; ACP agents skip Claude-subscription auto-assign. **Template limitation:** the runtime block's `command`/`args` are read on the `local:` resolve + deploy + `recreate_missing_container` paths only — the `github:` catalog path has never populated `template_data`, so ACP agents are effectively `local:`-template/API-config only until that gap closes.
